...
export function generateModelTSFiles(swagger: OpenAPIObject, options: GeneratorOptions) {
  ...
  const typeCollection = getTypeDefinitions(swagger, options);
...
function getTypeDefinitions(swagger: OpenAPIObject, options: GeneratorOptions) {
  ...
  forEach(swagger.components?.schemas, (item, key) => {
    if (!isInTypesToFilter(item, key, options)) {
      const type = getTypeDefinition(swagger, typeCollection, item, key, options);
      if (type) {
        typeCollection.push(type);
      }
    }
  });
  fillMissingBaseTypes(swagger, typeCollection, options);

  // fill types of the properties
  fillPropertyTypes(swagger, typeCollection, options);
...
function getTypeDefinition(
  ...
  if (isEnumType || item.type === 'integer') {
    return undefined;
  }
  ...
  const importFile = getImportFile(typeName, namespace, pathToRoot, isEnumType);
  ...
  if (isSubType) {
    ...
    if (baseType) {
      ...
      baseImportFile = getImportFile(baseType.typeName, baseType.namespace, pathToRoot, false);
  ...
  const type: ITypeMetaData = {
    fileName: getFileName(key, options, isEnumType),
    typeName,
    interfaceTypeName,
    fullNamespace,
    fullTypeName,
  ...
  PropertyHelpers.fillTypeProperties(swagger, type, required, properties, options, baseType);
  return type;
}

function fillMissingBaseTypes(swagger: OpenAPIObject, typeCollection: ITypeMetaData[], options: GeneratorOptions) {
  forEach(swagger.definitions, (item, key) => {
    ...
    if (isSubType && type && !type.baseType) {
      ...
      if (baseType) {
        ...
        baseType.isBaseType = true;
        // determine baseImportFile
        const baseImportFile = getImportFile(baseType.typeName, baseType.namespace, pathToRoot, false);
...
function fillPropertyTypes(swagger: OpenAPIObject, typeCollection: ITypeMetaData[], options: GeneratorOptions) {
...
function generateFormGroupFactories(namespaceGroups: INamespaceGroups, folder: string, options: GeneratorOptions) {
  ...
  for (const namespace in namespaceGroups) {
    if (namespaceGroups[namespace]) {
      ...
      let nrGeneratedFiles = 0;
      const exclusionProperties = options.propertiesToFilter || [
        'createdBy',
        'createdOnUtc',
        'updatedBy',
        'updatedOnUtc',
